...
export default function AtivarSetupModal({ tool, diagnosticId, clientEmail, clientName, onClose }: Props) {
  ...
  const whatsappInSetup = tool.setupFields.some((f) => f.key === "whatsapp_number");
  ...
  async function handleConfirmAndCheckout() {
    ...
    // 1. Save the pending activation record first so we have an ID to pass to Stripe
    const { data: activation, error: dbErr } = await supabase
    ...
    try {
      const successUrl = `${window.location.origin}${window.location.pathname}?ativar_success=1&activation_id=${activation.id}`;
      const cancelUrl = `${window.location.origin}${window.location.pathname}?ativar_cancel=1`;
      ...
      const resp = await fetch(`${SUPABASE_URL}/functions/v1/create-checkout`, {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ativar_activation_id: activation.id,
            tool_id: tool.id,
            tool_name: tool.name,
          },
        }),
      ...
      // Stripe checkout creation failed — show error
      setSaving(false);
      const detail = d?.detail || d?.error || "";
      setError("Erro ao iniciar pagamento." + (detail ? " Detalhe: " + String(detail).slice(0, 200) : ""));
    } catch (err) {
      console.error("[ativar-checkout] fetch failed", err);
      setSaving(false);
          ...
          {step === 1 && (
            <div>
              <p style={{ fontSize: 13, color: "#7a9ec8", lineHeight: 1.6, marginBottom: 24 }}>
                Preencha as informações abaixo. Elas serão usadas para configurar sua ferramenta após a confirmação do pagamento.
              </p>
          ...
          {step === 2 && (
            <div>
              <div style={{ background: "#102240", border: "1px solid #1e3a5f", borderRadius: 12, padding: 20, marginBottom: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: "#4a6fa0", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 12 }}>Resumo da configuração</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {tool.setupFields.map((f) => (
                    <div key={f.key} style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                      <span style={{ fontSize: 12, color: "#7a9ec8", flexShrink: 0 }}>{f.label}</span>
              ...
              <div style={{ background: "rgba(0,200,150,.06)", border: "1px solid rgba(0,200,150,.2)", borderRadius: 10, padding: "14px 16px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: 14, color: "#c8daf0" }}>Assinatura mensal</span>
                <span style={{ fontSize: 22, fontWeight: 700, color: "#00c896" }}>R${tool.priceMonthly}/mês</span>
              </div>
              <p style={{ fontSize: 12, color: "#4a6fa0", marginBottom: 20, textAlign: "center" }}>
                Cancele quando quiser, sem multa. Próximo passo: pagamento seguro via Stripe.
              </p>

              {!whatsappInSetup && (
                <div style={{ marginBottom: 20 }}>
                  <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#c8daf0", marginBottom: 6 }}>
                    Seu WhatsApp (para confirmação após ativação)
                  </label>
                  <input
                    type="tel"
                    value={whatsapp}
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button
                  ...
                  disabled={saving}
                  style={{ flex: 1, padding: "13px", background: "none", border: "1px solid #1e3a5f", borderRadius: 10, color: "#7a9ec8", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", opacity: saving ? 0.5 : 1 }}
                >
                  ← Voltar
                </button>
                <button
                ...
                >
                  {saving
                    ? <><Loader size={14} style={{ animation: "spin 1s linear infinite" }} /> Preparando pagamento...</>
                    : "Ir para pagamento →"}
                </button>
              </div>

              <p style={{ marginTop: 14, fontSize: 11, color: "#4a6fa0", textAlign: "center", lineHeight: 1.5 }}>
                Você será redirecionado para o Stripe, ambiente seguro de pagamento.
                Após confirmar, sua ferramenta será configurada em até 24h.
              </p>
            </div>
          )}

          {/* ── Step 3: Post-payment success (only shown when returning from Stripe) ── */}
          {step === 3 && (
            <div style={{ textAlign: "center", padding: "12px 0" }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                <CheckCircle size={64} color="#00c896" />
              </div>
              ...
              <p style={{ fontSize: 14, color: "#c8daf0", lineHeight: 1.7, marginBottom: 16 }}>
                Sua ferramenta está sendo configurada. Em até 24 horas você receberá uma mensagem no WhatsApp confirmando que está tudo pronto.
              </p>
